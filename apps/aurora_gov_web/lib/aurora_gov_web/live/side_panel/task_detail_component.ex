defmodule AuroraGov.Web.Live.Panel.Side.TaskDetail do
  ...
  def update(%{update: {type, _data}}, socket) do
    socket =
      if type in [
        :task_updated, :task_assigned, :task_completed, :task_abandoned, :task_cancelled,
        :effort_registered, :task_evaluated
      ] do
  ...
  defp load_task_details(socket) do
    ...
    |> start_async(:load_detail_data, fn ->
      ...
      person = if task && task.person_id do
      ...
      <% else %>
        <div class="border-b border-gray-100 pb-6 mb-6">
          ...
          <div class="mt-6 border-b border-gray-200">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
              <button phx-click="set_tab" phx-value-tab="general" phx-target={@myself} class={if @active_tab == "general", do: "border-aurora_orange text-aurora_orange whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm", else: "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"}>
                <i class="fa-solid fa-circle-info mr-2"></i>Información
              </button>
              <button phx-click="set_tab" phx-value-tab="history" phx-target={@myself} class={if @active_tab == "history", do: "border-aurora_orange text-aurora_orange whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm", else: "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"}>
                <i class="fa-solid fa-clock-rotate-left mr-2"></i>Historial
              </button>
              <button phx-click="set_tab" phx-value-tab="evaluation" phx-target={@myself} class={if @active_tab == "evaluation", do: "border-aurora_orange text-aurora_orange whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm", else: "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm"}>
        ...
        <div class="mt-6">
          <%= if @active_tab == "general" do %>
            <div class="space-y-6">
              <div>
                <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Objetivo</h3>
              ...
              <div class="flex flex-wrap items-center gap-6 pt-4 border-t border-gray-100">
                <%= if @assigned_person do %>
                  <div>
                    <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Asignado a</h3>
                ...
                <div>
                  <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Fechas</h3>
                  <div class="flex items-center gap-4 text-xs text-gray-500">
                    <span class="flex items-center gap-1.5"><i class="fa-solid fa-calendar-plus text-gray-400"></i> {Calendar.strftime(@task.created_at, "%d/%m/%Y")}</span>
                    <span class="flex items-center gap-1.5"><i class="fa-solid fa-clock text-gray-400"></i> {Calendar.strftime(@task.updated_at, "%d/%m/%Y")}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="space-y-4">
              <%= if Enum.empty?(@task_events) do %>
                <div class="bg-gray-50 rounded-xl p-8 border border-gray-200 text-center">
                  ...
                  <p class="text-xs text-gray-400 mt-1">No hay eventos registrados para esta tarea aún.</p>
                </div>
              <% else %>
                <div class="relative border-l border-gray-200 ml-4 space-y-6 pb-4">
                  <%= for event <- @task_events do %>
                    <div class="relative pl-6">
                      <div class="absolute -left-1.5 mt-1.5 w-3 h-3 bg-aurora_orange rounded-full border-2 border-white"></div>
                      <div class="bg-white border border-gray-100 rounded-lg p-3 shadow-sm">
                        <div class="flex justify-between items-start mb-2">
                          <h4 class="text-xs font-bold text-gray-800"><%= event.event_type |> String.split(".") |> List.last() %></h4>
                          <span class="text-[10px] text-gray-400"><%= Calendar.strftime(event.created_at, "%d/%m/%Y %H:%M:%S") %></span>
                        </div>
              ...
              <div class="bg-yellow-50/50 rounded-2xl border border-yellow-100 p-6 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <div class="flex flex-col items-end">
                    ...
                    <span class="text-3xl font-black text-yellow-600">{@task.evaluation_score} <span class="text-lg text-yellow-400">/ 100</span></span>
                  </div>
                </div>
                <div class="bg-white p-4 rounded-xl border border-yellow-100/60">
                  <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Reseña</h4>
            ...
            <% else %>
              ...
              <div class="bg-gray-50 rounded-xl p-8 border border-gray-200 text-center flex flex-col items-center">
                ...
                <%= if @task.status == :completed do %>
                  <p class="text-xs text-gray-500 mt-1 mb-4 max-w-md">La tarea ha sido completada pero aún no tiene una evaluación de desempeño registrada.</p>
                  <button
                    phx-click="open_proposal_create_modal"
                    phx-value-proposal_ou_origin={@app_context.current_ou_id}
                    phx-value-proposal_ou_end={@app_context.current_ou_id}
                ...
                <% else %>
                  <p class="text-xs text-gray-400 mt-1">La tarea debe estar en estado <b>Completada</b> para poder ser evaluada.</p>
                <% end %>
              </div>
            <% end %>
          <% end %>
